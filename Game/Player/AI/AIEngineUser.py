...
from Game.Pathfinding.AiMoveTreeLeaf import AiMoveTreeLeaf, AiMoveTreeHead, LeafResult
...
class AiEngineUser(IChessEngineUser):
    ...
    def input_player_move_input(self, paths: Dict[Tuple[int, int], PlayerPathDict]) -> Tuple[Vector2, Vector2, Vector2]:
        ...
        # PrintDebugger.print_board(self.board.map, self.board.game_board_size)
        best_move = self._find_best_move(paths)

        # for i in move_tree.leaves:
        #     # print(f'==========================================')
        #     # print(f'{i}')
        #     self.r(move_tree.leaves[i])
        # PrintDebugger.print_move_tree_pathsR(move_tree)
        # self._find_best_routeR(head_leaf, self.current_player_id)

        start_pos = best_move[0]
        end_pos = best_move[1]
        move_set = IChessEngineUser.get_move_set_vector(
            start_pos.get_tuple(),
            end_pos.get_tuple(),
            paths
        )
    ...
    def build_terminal_tree_node(
        ...
        move_tree = AiMoveTreeHead(self.max_tree_depth)
        bc = board.copy_board()
        for key in player_moves:
            value = player_moves[key]
            starting_position = value.position
            paths = value.paths
            for path_key in paths:
                path_dict = paths[path_key]
                for move_key in path_dict:
                    move_path: PathFindingTile = path_dict[move_key]
    ...
    def _find_best_route(self, head_leaf: AiMoveTreeHead, player_id: int) -> Tuple[Vector2, Vector2]:
        results = self._find_best_leaf(head_leaf.leaves, player_id)
        return results[0], results[1]

    def _find_best_leaf(self, leaves: Dict[Tuple[int, int], AiMoveTreeLeaf], player_id: int):
    ...
    def _evaluate_leaf_score_two(self,
                                 leaves: [AiMoveTreeLeaf],
                                 results: Dict[int, LeafResult] = None,
                                 parent_leaf_result: LeafResult = None,
                                 offset_index: int = 0) -> Dict[int, LeafResult]:
        if results is None:
            results = {}

        for index, leaf_key in enumerate(leaves):
            leaf: AiMoveTreeLeaf = leaves[leaf_key]
    ...
    def _evaluate_leaf_score(self, head_leaf: AiMoveTreeLeaf, score: LeafResult = None) -> LeafResult:
        if score is None:
    ...
    @staticmethod
    def prune_move_leaves(
            leaves: Dict[Tuple[int, int], AiMoveTreeLeaf]
    ):
        og_leaves: Dict[Tuple[int, int], AiMoveTreeLeaf] = leaves.copy()
        p1 = -1000000
        p2 = 1000000
        keys_to_preserve = []
